...
### 優先対応
- [ ] 保存先設定 UI をヘッダ直下へ再配置し、設定済み／未設定の表示と変更・リセットボタン、注釈を統合する。  
  ↳ レイアウト・スタイルは現行テイストに合わせて実装。
- [ ] `layout_settings.json` の `name` をプロファイル表示名と出力ファイル名に分離し、後から編集できるようにする。
- [ ] プロファイル管理機能を拡張し、追加（モーダル入力: 表示名／出力ファイル名／幅／高さ／初期出力形式）と削除（確認ダイアログあり）、localStorage 永続化を実装する。
- [ ] 設定タブ内でデバッグモードを扱えるようにレイアウトを整理し、タブ構成を現行 UI 方針に沿って再配置する。
- [ ] レイアウトパターンの「現在の設定概要」を「出力設定一覧」に改称し、形式が未選択の場合は「ファイルは出力されません」と表示する。
